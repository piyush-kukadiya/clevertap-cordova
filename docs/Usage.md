...
#### Fetch Inbox (on-demand refresh)

```javascript
this.clevertap.fetchInbox(success => {
    console.log('fetchInbox success: ' + success);
});
```
...
```javascript
this.clevertap.pushDisplayUnitElementClickedEventForID('unit_id_1234', { wzrk_element_id: 'btn_01' });
```
